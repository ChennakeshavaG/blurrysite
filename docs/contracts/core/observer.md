...
| `_engineNodeBuffer` | `Node[]` | Filled by MO callbacks, drained per idle. |
| `_engineWorkRemainder` | `Element[]` | Flat work items (elements) carried across idle ticks when chunked engine drain yields. |
| `ENGINE_CHUNK_SIZE` | `number` (500) | Elements processed per idle tick. ~5x cheaper than PII text nodes (no regex/splitText), so larger chunk than PII's 200. |
| `_subscribers` | `Map<string, fn>` | Insertion-order preserved. |
| `_subscriberRecordBuffer` | `Map<root, MutationRecord[]>` | Cleared each idle after engine nodes finish. |
| `_stampProcessScheduled` / `_processScheduled` | `boolean` | Single-flight idle gates. |
...
- **Ancestor-coverage filter**: nodes whose subtree is already covered by an ancestor in the same batch are dropped before stamping via Set-based parent walk (O(n × depth) instead of O(n²)). Prevents double-walking when a SPA inserts a container in one MO tick and its children in another before the idle fires.
- **Chunked engine drain**: `_processObservedChanges` flattens deduped top-level nodes + their descendants into a single work array via `_flattenForProcessing`, then processes `ENGINE_CHUNK_SIZE` (500) elements per idle tick via `_processEngineChunk`. Unprocessed elements are saved in `_engineWorkRemainder` for the next tick. Subscriber dispatch is deferred until all engine work completes. A tail check after dispatch reschedules if new MO data arrived during the yield window.
- **PII span skip**: `_collectEngineNodes` skips elements with `data-bl-si-pii` — these are PII wrapper `<span>`s generated by `pii.js` `_wrapTextNode`. The engine's competing-blur guard in `_evaluateAndStamp` would also skip them, but filtering at buffer time avoids the idle drain overhead on pages with hundreds of PII wraps per scan chunk.
