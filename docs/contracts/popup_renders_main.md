...
3. **Screen-share sub-card** — when `triggers.screen_share` is active, screen-share is suppressed (tab/host), or screen-share is suspended (`settings.screen_share_suspended` AND `ssShareLive`). Priority: suspended > suppressed > triggered. If suspended: shows "Suspended — resumes on restart" label + Resume/Undo button calling `ctx.onUnsuppressScreenShare('feature')`. If suppressed: shows undo row only, no actions. If active: trigger label + elapsed timer + 3-button action row. Suspended card gated on `ssShareLive` because `set_screen_share_inactive` auto-resumes suspension when no shares remain — the guard is defense-in-depth against stale session state.
