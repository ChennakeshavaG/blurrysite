...
4. **Strength slider** (range 2–20) — hidden when active type is `redacted`, `censored`, or `color`. Writes `global_default_settings.blur_radius`. Slider value badge displays a tier label (`htb_strength_subtle` 2–6, `htb_strength_moderate` 7–13, else `htb_strength_strong` 14–20) — never the raw pixel number. Range floor of 2 mirrors `validate_model`'s `blur_radius >= 2` clamp in `src/constants.js` — values below 2 are silently rejected by storage validation, so the UI must never emit them.
