...
Per-module contracts: `docs/contracts/<module>.md` (one file per module — read this during implementation).
Full design reference: `docs/module-contracts.md` and `docs/architecture.md` (planning only — see rule 4 below).
...
2. Check `docs/contracts/<module>.md` for the contract of the module you're touching — do not introduce new public API surface without updating that file.
...
4. Do NOT read or search `docs/` during implementation — contracts are in `docs/contracts/<module>.md`. Only open other `docs/` files when explicitly planning a new feature.
...
| `src/blur_engine.js` | `blsi.BlurEngine` | Low-level: `applyBlur`, `removeBlur`, `unblurAll`, `isBlurred`, `isVisuallyBlurred`, `injectRules`, `removeRules`, `isBlurAllActive`, `stampElements` (returns `ShadowRoot[]`), `tryBlurTextCheck`, `matchesActiveCategories`, `shouldBlurElement`, `ensureSvgFilter`, `injectPiiRules(mode)`, `removePiiRules()`, `getZoneOverlays`, `teardown`, `CATEGORY_SELECTORS`. High-level: `handleSite`, `handleShadowRoot`, `handleIframe`, `observeRoot`, `resetCounters`, `allocateDynamicName`, `allocateStickyName`, `isPageBlurred` (getter), `blurredCount` (getter — O(1) running count of `data-bl-si-blur` stamped elements), `_setPickerActiveForObserver` |
...
| `src/main_world_bridge.js` | _(none — MAIN world)_ | No global; runs at `document_start` in page's MAIN world. Patches `navigator.mediaDevices.getDisplayMedia` (fires `'__blsi_screen_share'` on share start/stop) and `Element.prototype.attachShadow` (fires `'__blsi_shadow_attached'` on the element for late shadow root discovery). No `chrome.*` access. |
...
**Popup namespace rule:** All popup code (render files, `popup.js`, `popup_state.js`) uses the exact `blsi.DEFAULT_MODEL` nested shape — never a flat alias. The old flat keys (`blur_mode`, `pick_blur_enabled`, `pick_blur_type`, `pii_email`, `pii_numeric`, `blur_radius`, `reveal_mode`, `redaction_color`, `thorough_blur`, `transition_duration`, `blur_categories`, `pick_blur_color`, `automate_idle`, `automate_tab_switch`, `automate_screen_share`, `enabled`) are removed. Use the nested model path everywhere (`settings.blur_all.settings.blur_mode`, `settings.global_default_settings.enabled`, etc.). `saveSettings(patch)` in `popup_state.js` takes a model-shaped patch and routes each top-level key via `patch_section`.
...
### Running tests
```bash
npm run test:unit          # 714 unit tests, fast
npm test                   # + coverage (~91% line coverage on src/)
```

**Do NOT read test files unless you are modifying tests.** To understand behavior, read `docs/contracts/<module>.md` — tests confirm correctness, contracts document intent. Loading tests pre-emptively costs ~5K tokens with no benefit for implementation tasks.
...
| What changed | Update |
|---|---|
| Added/removed/renamed a public API method on any module | `CLAUDE.md` Module Globals table, `src/CLAUDE.md` module-specific rules, `docs/contracts/<module>.md` contract |
...
| Added/removed/renamed a `blsi.Model` method or storage key | `CLAUDE.md` Module Globals table (`src/storage_model.js` row), `src/CLAUDE.md` storage_model.js rules, `docs/contracts/storage_model.md` contract |
