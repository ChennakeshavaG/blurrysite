...
| `src/pii/pii_state.js` | `blsi.PiiState` | `PII_ATTR` constant + match-count + active-types + per-scan country signal cache (Phase 4 — `getCountry`/`setCountry`/`clearCountry`) + regex cache + scan stats. Shared private state for the PII sub-modules. |
| `src/pii/pii_checksums.js` | `blsi.PiiChecksums` | `luhn`, `verhoeff`, `mod97`, `mod11Weighted`, `iso7064Mod11_2`, `isbn13`, `isbn10`. Pure-math validators consumed by Stage 1 + Stage 2 detectors. `bech32` / `base58check` / `mod89` / `iso7064Mod11_10` / letter-tables deferred (no detector requires them yet). |
| `src/pii/pii_pre_filter.js` | `blsi.PiiPreFilter` | `isExtensionUI(node)`, `isInsidePiiSpan(node)`, `isInsideCodeBlock(node)`, `hasDigit(text)`, `hasDigitOrLongAlnum(text)` — Stage 0 whole-node drops. The `hasDigitOrLongAlnum` extended pre-screen passes nodes whose only PII signal is a long alpha-only token (Bearer headers, base64) — used by the numeric branch to feed the identifier sub-pass. |
| `src/pii/pii_country.js` | `blsi.PiiCountry` | Phase 4 — `detect()` (cached, reads document) / `detectFromInputs(inputs)` (pure) / `_resetCache()`. Returns ISO 3166 alpha-2 or null from priority chain: meta tags (`geo.country`, `og:locale`, `content-language`) → `<html lang>` (region subtag required) → ccTLD allow-list (~45 entries) → currency-density (single-country symbols only: £/₹/₩/₽). Cache is M6 from PERF.md. |
| `src/pii/pii_suppressors.js` | `blsi.PiiSuppressors` | `NUMERIC_PROFILE`, `FALSE_POSITIVE_CHECKS`, `falsePositivesCheck`, plus 14 cascade suppressors: `isYear`, `isVersion`, `isHexColor`, `isYearRange`, `isPercentage`, `isScientificNotation`, `isMeasurement`, `isResolution`, `isOrdinalLabel`, `isDateLike`, `isOrderRef`, `isStatistic`, `isPublicPrice`, `isCountNoise`. Stage 4 FP cascade ordered cheap-to-expensive with short-circuit between tiers. |
| `src/pii/pii_detectors.js` | `blsi.PiiDetectors` | `EMAIL_RE`, `NUMERIC_RE` (7 alts), `PATTERNS`, `STAGE1_DETECTORS`, `STAGE2_DETECTORS`, `findMatches(text, types)`, `getPatterns()`. **Phase 5 consolidation**: every detector is a frozen data row driven by a single `_runDescriptor` runner — descriptor shape `{ id, regex, checksum?, dispositive?, countries?, keywordRe?, keywordWindow?, action, preScreen? }`. Adding a detector is a row append. `findMatches` runs four layers: (1) Stage 1 dispositive — Card PAN, IBAN, ETH, ISBN-13 (suppress), Aadhaar, CN ID, NRIC SG, CURP MX, Emirates ID, NIE ES, Codice Fiscale, UK / CA postal, IPv6, GPS DMS, Plus Code; (2) identifier sub-pass (Bearer / AKIA / ghp_ / sk_ / pk_ / AIza / xox- / JWT + KEYWORD-value capture); (3) Stage 2 context-gated — MAC, IPv4, IMEI, E.164, SSN_US, NHS_UK, BSN_NL, NPI_US, DNI_ES, ABN_AU, MRN, postal_jp/au/nl/br, us_zip4, eircode_ie; (4) Stage 3 NUMERIC_RE + Stage 4 FP cascade. All four layers share `consumed[]`; SUPPRESS hits push without emitting (anti-PII). Users see one `numeric` toggle; per-detector behaviour configured by maintainers via descriptor rows — no popup UI per detector. |
...
### Running tests
```bash
npm run test:unit          # 957 unit tests, fast
